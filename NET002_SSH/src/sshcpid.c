/*
 * sshcpid — SSH copy-id test for FujiNet
 *
 * Installs the FujiNet public key on a remote server using
 * N:SSH.COPYID://user:pass@host:port/
 *
 * Hardcoded: host=192.168.1.20, user=test, pass=test, port=22
 *
 * Build:  cl65 -t atari -o build/sshcpid.com src/sshcpid.c
 *
 * Requires: FujiNet firmware with SSH.COPYID protocol support
 *           and /.ssh/id_ed25519.pub on SD card.
 *
...
/* Hardcoded target                                                   */
/* ------------------------------------------------------------------ */
#define TARGET_HOST "192.168.1.20"
#define TARGET_USER "test"
#define TARGET_PASS "test"
#define TARGET_PORT "22"

static unsigned char devicespec[] =
    "N:SSH.COPYID://" TARGET_USER ":" TARGET_PASS
    "@" TARGET_HOST ":" TARGET_PORT "/\x9b";
...
int main(void)
{
    ...
    printf("=== FujiNet SSH Copy-ID ===\n\n");
    printf("Host: " TARGET_HOST "\n");
    printf("User: " TARGET_USER "\n");
    printf("Port: " TARGET_PORT "\n\n");
    printf("Installing public key...\n");

    /* OPEN — triggers SSH.COPYID on FujiNet */
    POKE(DTIMLO, 60);  /* longer timeout for remote connection */
    sts = net_open();
    if (sts != 1) {
        printf("\nOPEN failed (SIO:%u)\n", sts);
